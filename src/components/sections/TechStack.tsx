...
function TechCard({
  ...
  const glow = useMotionTemplate`radial-gradient(280px circle at ${mouseX}px ${mouseY}px, var(--glow), transparent 55%)`;
    ...
    <StaggerItem>
      ...
      >
        <motion.article
          ...
          className="group relative flex h-full min-h-[176px] flex-col overflow-hidden rounded-[22px] border border-accent/15 bg-elevated/85 p-5 shadow-[0_18px_36px_-24px_var(--shadow-color)] backdrop-blur-xl"
        >
          <motion.div
          ...
          <div
            ...
            style={{
              boxShadow:
                "inset 0 0 0 1px var(--glow), 0 0 28px -8px var(--glow)",
            }}
            aria-hidden
          />
          ...
          <div className="relative z-[1] flex flex-1 flex-col">
            <div className="mb-4 flex items-start justify-between gap-3">
              <motion.div
                ...
                className="flex h-11 w-11 items-center justify-center rounded-xl border border-border-subtle bg-soft p-2 transition-colors duration-300 group-hover:border-accent/30 group-hover:bg-soft"
              >
                <Logo className="h-6 w-6" />
              </motion.div>
              <span className="rounded-full border border-accent/20 bg-accent/5 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-champagne">
                {tech.category}
              </span>
            </div>

            <div className="mb-1 flex items-center gap-2">
              <h3 className="font-display text-[1.05rem] font-bold tracking-tight text-heading">
                {tech.name}
              </h3>
              <span className="rounded-md bg-soft px-1.5 py-0.5 text-[10px] text-muted">
                {tech.tag}
              </span>
            </div>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 60% 45% at 50% 20%, var(--glow), transparent 70%)",
        }}
      />
      ...
      <div className="container-premium relative z-[1]">
        <SectionHeading
          ...
          description="The tools I use every day at work — React, Next.js, Node, MongoDB, and more."
          align="center"
          className="mx-auto"
        />
